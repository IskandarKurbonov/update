...
for REPO in ${ONLYOFFICE_REPOS[*]}; do
    ...
    while read -r file; do
        if grep -q "Copyright Ascensio System SIA 2024" "$file"; then
            ...
            perl -i -0777 -pe 's|"""\s*.*?Copyright Ascensio System SIA 2024.*?"""||gs' "$file"
            perl -i -0777 -pe 's|<!--\s*.*?Copyright Ascensio System SIA 2024.*?-->||gs' "$file"
            perl -i -0777 -pe 's|^#\s*.*?Copyright Ascensio System SIA 2024.*?$(\n^#.*$)*||gsm' "$file"
            perl -i -0777 -pe 's|=begin\s*.*?Copyright Ascensio System SIA 2024.*?=end||gs' "$file"
            perl -i -0777 -pe 's|//\s*.*?Copyright Ascensio System SIA 2024.*?$(\n//.*$)*||gsm' "$file"
            perl -i -0777 -pe 's|REM\s*.*?Copyright Ascensio System SIA 2024.*?$(\nREM.*$)*||gsm' "$file"
        fi
